...
struct peak{
	...
	peak(int i_rho, int i_theta, int i_hval){
		rho = i_rho;
		theta = i_theta;
		hval = i_hval;
	}
...
Mat_<int> lab3computeHough(Mat_<uchar> image){
	int diag = sqrt(pow(image.rows, 2) + pow(image.cols, 2));
	Mat_<int> houghAccumulator(diag + 1, 360);
	houghAccumulator.setTo(0);
	std::vector<Point2d> points;
	for (int i = 0; i < image.rows; i++){
		for (int j = 0; j < image.cols; j++){
			if (image(i, j) == 255){
	...
	for (Point2d p : points){
		for (int theta = 0; theta < 360; theta++){
			float angle = ((float)theta / 180) * PI;
			int rho = p.x * cos(angle) + p.y * sin(angle);
			if (rho >= 0 && rho < diag + 1){
				houghAccumulator(rho, theta)++;
			}
		}
	}
...
std::vector<peak> lab3getPeaks(int n, int k, Mat_<uchar>image){
	Mat_<int> hough = lab3computeHough(image);
	std::vector<peak> localMaximas;
	for (int i = 0; i < hough.rows - n/2; i++){
		for (int j = 0; j < hough.cols - n/2; j++){
			int max = 1;
			for (int k = -n / 2; k <= n / 2; k++)
				for (int l = -n / 2; l <= n / 2; l++) {
					if (i + k > 0 && i + k < hough.rows && j + l > 0 && j + l < hough.cols){
						if (hough(i, j) < hough(i + k, j + l)){
							max = 0;
						}
					}
				}
			if (max && hough(i, j) > 5){
				localMaximas.push_back(peak(i, j, hough(i, j)));
			}
		}
	}

	std::sort(localMaximas.begin(), localMaximas.end());
	std::vector<peak> returnValue(localMaximas.begin(), localMaximas.begin() + k);

	return returnValue;
}

void lab3drawHoughLines(int n, int k){
	Mat_<uchar> image = readImage();
	std::vector<peak> maximas = lab3getPeaks(n, k, image);

	Mat_<Vec3b> result;
	cvtColor(image, result, COLOR_GRAY2RGB);

	for (peak p : maximas){
		float theta = (p.theta / PI) * 180;
		double a = cos(theta), b = sin(theta);
		double x0 = a*p.rho, y0 = b*p.rho;
		int x = a*p.rho;
		int y = b*p.rho;
		int x1 = a;
		int y1 = cvRound(y0 - 1000 * (a));

		line(result, Point(x0, y0), Point(x1, y1), Scalar(0, 255, 0));
	}
	imshow("Hough Lines", result);
	waitKey();
}

Mat_<uchar> lab4drawDistanceTransform(){
	Mat_<uchar> image = readImage();
	...
	for (int i = 1; i < image.rows; i++){
		for (int j = 1; j < image.cols - 1; j++){
			for (int k = 0; k < 4; k++){
				if (image(i + di[k], j + dj[k]) + weight[k] < image(i, j)){
					image(i, j) = image(i + di[k], j + dj[k]) + weight[k];
				}
			}
		}
	}
...
Mat_<uchar> lab4drawDistanceTransform(Mat_<uchar> image){
	int di[8] = { -1, -1, -1, 0, 0, 1, 1, 1 };
	int dj[8] = { -1, 0, 1, -1, 1, -1, 0, 1 };
	int weight[8] = { 3, 2, 3, 2, 2, 3, 2, 3 };
...
void lab4MatchingScore(){
	Mat_<uchar> templateImage = lab4drawDistanceTransform();
	Mat_<uchar> image = readImage();
	float sum = 0;
	int pixels = 0;
...
void lab4MatchingScore(Mat_<uchar> templateImage, Mat_<uchar> image){
	float sum = 0;
	int pixels = 0;
...
void lab4MatchingScoreCentered(){
	Mat_<uchar> templateImage = readImage();
	Mat_<uchar> image = readImage();
	...
	templateImage = lab4drawDistanceTransform(templateImage);
	lab4MatchingScore(templateImage, image);
}
...
int main()
{
	...
	do
	{
		...
		printf("Menu:\n");
		printf(" 1 - Draw DT\n");
		printf(" 2 - Mathcing Score\n");
		printf(" 3 - Centered Mathcing Score\n");
		printf(" 0 - Exit\n\n");
		printf("Option: ");
		scanf("%d",&op);
		switch (op)
