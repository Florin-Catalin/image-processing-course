	...
	while (openFileDlg(fname))
	{
		...
		for(int i=0;i<src.rows;i++)
			for (int j = 0; j < src.cols; j++) {
				...
				fi.at<float>(i, j) = atan2(Gx.at<float>(i,j), Gy.at<float>(i,j));
			}
...
int determine_area(float fi) {
	...
	if (fi >= (-CV_PI / 8) && fi <= (CV_PI / 8)) return 0;
	else
	if (fi >= (CV_PI / 8) && fi <= (3 * CV_PI / 8)) return 1;
	else
	if (fi >= (3 * CV_PI / 8) && fi <= (5 * CV_PI / 8)) return 2;
	else
	if (fi >= (5 * CV_PI / 8) && fi <= (7 * CV_PI / 8)) return 3;
	else
	if ((fi >= (7 * CV_PI / 8) && fi <= CV_PI) || (fi >= -CV_PI) && fi <=(-7 * CV_PI / 8)) return 0;
	else
	if (fi >= (-7 * CV_PI / 8) && fi <= (-5 * CV_PI / 8)) return 1;
	else
	if (fi >= (-5 * CV_PI / 8) && fi <= (-3 * CV_PI / 8)) return 2;
	else
	if (fi >= (-3 * CV_PI / 8) && fi <= (-CV_PI / 8)) return 3;
